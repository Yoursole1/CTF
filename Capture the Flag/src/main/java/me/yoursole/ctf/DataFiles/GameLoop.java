...
public class GameLoop {
    ...
    public GameLoop() {
        taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(Objects.requireNonNull(Bukkit.getPluginManager().getPlugin("CapturetheFlag")), new Runnable() {
            @Override
            public void run() {
                if (GameData.it != null) {
                ...
                if (GameData.timerMs != -1L) {
                    ...
                    for (Player player : Bukkit.getOnlinePlayers()) {
                        ...
                        if (GameData.it != null && GameData.it != player) {
                            Location itLoc = Bukkit.getPlayer(GameData.it.getUniqueId()).getLocation();
                        ...
                        text.setText(ChatColor.GREEN + flagHolder + " has the flag!" + ChatColor.WHITE + ChatColor.BOLD + " " + String.valueOf(arrow) + " " + ChatColor.AQUA + timeString);
